 ...
 *   Rating: 2
 */
int onlyOne(int x, int y) {//solved
 ...
 *   Rating: 2
 */
int subOK(int x, int y) {//solved
  int first_x = (x >> 31) & 1;
  int first_y = (y >> 31) & 1;
  int first_x_sub_y = ((x + (~y) + 1) >> 31) & 1;
  int different = first_x ^ first_y;
  return (!different) | (!(first_x ^ first_x_sub_y));
}
/*
...
int theMax(int x, int y) {
  ...
  int first_x_sub_y = ((x + (~y) + 1) >> 31) & 1;
  int different = first_x ^ first_y;
  int x_is_bigger = (different & (!first_x)) | ((!different) & (!first_x_sub_y));
  int markx = (x_is_bigger << 31) >> 31;
  return (x & markx) + (y & (~markx));
}
/*
 ...
 *   Rating: 4
 */
int distance(int x, int y) {//solved
...
{
  int mark = x >> 31;
  int abs_x = (x ^ mark) + (mark & 1) + 1;
  int ffff = 0xff | 0xff << 8;
  int high = (abs_x >> 16)&ffff, low = abs_x & ffff;
  int high_part, low_part, ans;
  high = high + (high << 2);
  high = high + (high << 4);
  high = high + (high << 8);
  low = low + (low << 2);
  ...
  low = low + (low << 8);
  high_part = high;
  low_part = (low + (low >> 16) + high) >> 16;
  ans = high_part + low_part;
  ans = ans >> 1;
  return (ans ^ mark) + (mark & 1);
}
/*
...
int modThree(int x) {//solved
  ...
  int flag = x >> 31;
  int sum_all_4 = sum_all_3 + flag;
  int is_3 = !(sum_all_4 ^ 3);
  int low_0 = flag & 1;
  int mark = is_3 | (low_0 & (!!sum_all_4));
  return sum_all_4 + (~(3 & ((mark << 1) | mark)) + 1);
}
/*
 ...
 *   Rating: 3
 */
int float_f2i(unsigned uf) {//solved
 ...
 *   Rating: 3
 */
unsigned float_half(unsigned uf) {//solved
  unsigned sign = (uf >> 31) & 1;
  int  exp = (uf >> 23) & 0xff;
  unsigned frac = uf & 0xffffff;
  unsigned last_frac = uf & 1;
  unsigned last;
  if (exp == 0xff)
    return uf;
  else
  {
    if (exp > 1) //feiguigehua
    {
      //    printf("thie\n");
      exp -= 1;
...
unsigned float_construct(int x, int y) {
  ...
  {
    exp += y;
    return uf | (exp << 23) | (frac & 0x7fffff);
  }
  else
  {
    n = 1 - (exp + y);
    ...
    if (n <= 24)
    {
      ...
      if (last > (1 << (n - 1)))
        frac += 1;
      else if (last == (1 << (n - 1)) && last_)
        frac += 1;
    }
    else
      frac = 0;
    return uf | (exp << 23) | frac;
  }
}